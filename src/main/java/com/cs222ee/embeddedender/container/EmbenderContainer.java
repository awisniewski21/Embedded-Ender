...
public class EmbenderContainer extends Container {
    private final TileEntity tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;

    public EmbenderContainer(int WindowID, World world, BlockPos pos,
                             PlayerInventory playerInventory, PlayerEntity player) {

        super(ModContainers.EMBENDER_CONTAINER.get(), WindowID);
        this.tileEntity = world.getTileEntity(pos);
        playerEntity = player;
        this.playerInventory = new InvWrapper(playerInventory);
        // Creates the normal player inventory at the bottom
        layoutPlayerInventorySlots(8, 86);

        // Create slots for the Embender
        if(tileEntity != null){
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()),
                playerIn, ModBlocks.EMBENDER.get());
    }

    /***
     * Taken from KaupenJoes's tutorial
     * Source: https://gist.github.com/Kaupenjoe/cbb03a4db3f94a2bfd5b2ad2fa78f539
     ***/
